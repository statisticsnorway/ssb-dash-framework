...
skjemamottak = conn.query("SELECT * FROM skjemamottak", partition_select={"aar": [_t_0, _t_1]})
skjemadata = conn.query("SELECT * FROM skjemadata_hoved", partition_select={"aar": [_t_0, _t_1]})
datatyper = conn.query("SELECT * FROM datatyper", partition_select={"aar": [_t_0, _t_1]})
...
skjemadata_tbl = (
    skjemadata_tbl
    .filter(skjemadata_tbl.refnr.isin(relevant_refnr))
    ...
    .mutate(
        ...
        pdiff=lambda t: (t[_t_0].fill_null(0) - t[_t_1].fill_null(0)) / t[_t_1].fill_null(1) * 100    
    )
)
