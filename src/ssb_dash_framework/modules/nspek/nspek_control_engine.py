...
from .nspek_control_config import CONTROL_RULES, get_controls_for_field, get_rule_by_id
...
def get_active_versions(conn, aar: int) -> pd.DataFrame:
    ...
    df_versions = (
        ...
        .select(_.orgnr, _.aar, _.sekvensnummer, _.versjon_nr, _.antall_versjoner, _.dato_mottatt)
        .execute()
    )
    ...
    df_updates = (
        t_updates
        .filter(_.sekvensnummer.isin(df_versions["sekvensnummer"].tolist()))
        .execute()
    )
...
def get_regnskaps_data(conn, scope_df: pd.DataFrame, regnskapstype: str) -> pd.DataFrame:
    ...
    df = (
        t.filter(_.sekvensnummer.isin(sekvensnummer_liste))
        .execute()
    )
    ...
    df_wide = df_wide.merge(scope_df[["sekvensnummer", "orgnr", "aar"]], on="sekvensnummer", how="left")
        ...
        [
            {
                ...
                "sortering": "DESC",
                "sist_kjoert": datetime.now(timezone.utc),
            }
...
def insert_batches(conn, table_name: str, columns: list[str], rows: list[dict], chunk_size: int = 5000) -> None:
    if not rows:
        return
    ...
    for start in range(0, len(rows), chunk_size):
        ...
        values_sql = ",".join(
            "(" + ",".join(sql_value(v) for v in row.values()) + ")"
            for row in chunk
        )
...
def save_full_control_db(conn: BaseBackend, aar: int, df_kontroller: pd.DataFrame, df_kontrollutslag: pd.DataFrame) -> None:
    print(f"Sletter og laster til kontrollutslag og kontroll tabellene.")
...
def save_incremental_control_db(conn: BaseBackend, sekvensnummer: int, kontrollids: list[str], df_kontrollutslag: pd.DataFrame):

    if not kontrollids:
        return

    kontrollids_sql = ",".join(
        sql_value(x)
        for x in kontrollids
    )
    try:
        conn.raw_sql("BEGIN;")
...
def run_all_controls(df_resultat: pd.DataFrame, df_balanse: pd.DataFrame) -> pd.DataFrame:
    all_results = []
...
def run_controls_for_changed_fields(changed_fields: list[str], df_resultat: pd.DataFrame, df_balanse: pd.DataFrame) -> pd.DataFrame:
    kontrollids = set()
...
def run_all_controls_for_sekvensnummer(conn: BaseBackend, sekvensnummer: int) -> pd.DataFrame:
